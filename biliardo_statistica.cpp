...
StatsResult simulate_stats (
  ...
  double L
    ...
    auto parameters = [](const std::vector<double>& data) -> Stats {
        ...
        double variance = std::accumulate(data.begin(), data.end(), 0.0, [mean](double acc, double val) {
        ...
        if (stdev != 0.0) {
            coeff_simm = std::accumulate(data.begin(), data.end(), 0.0, [mean, stdev](double acc, double val) {
                    double z = (val - mean) / stdev;
                    return acc + z * z * z;
                }
            ) / n;
            
            coeff_app = std::accumulate(data.begin(), data.end(), 0.0, [mean, stdev](double acc, double val) {
                    double z = (val - mean) / stdev;
                    return acc + z * z * z * z;
                }
            ) / n;

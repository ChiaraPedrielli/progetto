...
int main(){
   try{
    ...
    std::cout << "La linea contrale corrisponde all'asse x, l'asse y si trova sul alto sinistro della finestra.\n Inserire l'ordinata della pallina e l'angolo di lancio in radianti: i valori concessi sono da -1.55 a +1,55\n";
    std::cin >> y >> angle;
    ball.move_to({0, y + 300});
    ball.set_angle(angle);
    
    
    std::cout << "Si definiscono ora le coordinate dei bordi. Inserire, nell'ordine: ornidata dell'estremio sinistro, ascissa dell'estremo destro e ordinata dell'estremo destro";
    std::cin >> r1 >> L >> r2; 
    if(r1 >= 0 && r2 >= 0){
